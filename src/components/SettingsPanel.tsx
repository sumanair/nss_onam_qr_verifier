...
export default function SettingsPanel({ verifierId, setVerifierId, apiKey, setApiKey, note, setNote, onLogout }:{
  ...
  return (
    <div className="settings card">
      ...
      <div className="row"><label className="lbl">API Key {REQUIRE_KEY?"(required)":"(optional)"}</label><input className="inp" type="password" placeholder="paste key" value={apiKey} onChange={e=>setApiKey(e.target.value)} /></div>
      <div className="row"><label className="lbl">Note</label><input className="inp" type="text" placeholder="optional (gate, reason, etc.)" value={note} onChange={e=>setNote(e.target.value)} /></div>
